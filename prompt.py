...
class BasePrompt:
    ...
    def summarize_file_diff(self):
        return self._format(self._SUMMARIZE_FILE_DIFF + self._TRIAGE_FILE_DIFF)  # noqa: E501

    @property
    def summarize_changesets(self):
        return self._format(self._SUMMARIZE_CHANGESETS)

    @property
    def summarize(self):
        return self._format(self._SUMMARIZE_PREFIX + self._SUMMARIZE)

    @property
    def summarize_short(self):
        return self._format(self._SUMMARIZE_PREFIX + self._SUMMARIZE_SHORT)

    @property
    def summarize_release_notes(self):
        return self._format(self._SUMMARIZE_PREFIX + self._SUMMARIZE_RELEASE_NOTES)  # noqa: E501

    @property
    def review_file_diff(self):
        return self._format(self._REVIEW_FILE_DIFF)

    @property
    def comment(self):
        return self._format(self._COMMENT)
...
- **Poem**: Below the changes, include a whimsical, short poem written by a rabbit to celebrate the changes. Format the poem as a quote using the ">" symbol and feel free to use emojis where relevant.

Avoid additional commentary as this summary will be added as a comment on the GitHub pull request. Use the titles "Walkthrough" and "Changes" and they must be H2.
        ''',  # noqa[E501]
    }
